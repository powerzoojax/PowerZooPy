...
training_battery_env_config = {
    'name': 'Case29GB-Battery',
    'description': 'Battery arbitrage',
    'grid': {
        'type': 'transmission',
        'case': 'Case29GB',
        'start_date': '2025-01-01',
        'end_date': '2025-12-31',  # 1 year for training
        'delta_t_minutes': 60,  # 1-hour intervals
        'max_load_ratio': 0.9, # Annual peak load == 0.9 * Total capacity
    },
    'resources': [
        {'type': 'solar', 'capacity_mw': 50, 'bus_id': 2},
        {'type': 'wind', 'capacity_mw': 100, 'bus_id': 3},
        {'type': 'battery', 'name': 'bat0', 
         'capacity_mw': 50, 'power_mw': 20, 'bus_id': 4,
...
print("✓ Using PowerEnv with training_battery_env_config")
...
if HAS_SB3:
    try:
        ...
        model_dir = Path(f"{os.path.dirname(__file__)}/x_RL05_GB_output")
        model_dir.mkdir(exist_ok=True)
        model_path = model_dir / "sac_battery_model_gb.zip"
...
output_dir = 'examples/x_RL05_GB_output'
os.makedirs(output_dir, exist_ok=True)
output_path = os.path.join(output_dir, 'battery_control_sb3_gb.png')
